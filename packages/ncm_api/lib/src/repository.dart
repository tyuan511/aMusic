...
class Repository {
  ...
  Future<int> loginQrCheck(String key) async {
    try {
      final ret = await (await doRequest('/login/qr/check', {'key': key})).asFuture;
  ...
  Future<Result<SearchResultSongs>> searchSongs(
    ...
    final result = await search(keyword, SearchType.song, limit: limit, offset: offset);
  ...
  Future<Result<List<String>>> searchSuggest(String? keyword) async {
    ...
    return _map(response, (dynamic t) {
      final match = (response.asValue!.value['result']['allMatch'] as List?)?.cast();
      if (match == null) {
        return [];
      }
  ...
  Future<Result<List<Album>>> artistAlbums(
    ...
    return _map(result, (t) {
      ...
      return hotAlbums.cast<Map<String, dynamic>>().map((e) => Album.fromJson(e)).toList();
    });
  }
  ...
  Future<Result<List<PlayRecord>>> getRecord(
    ...
    final result = await doRequest('/user/record', {'uid': uid, 'type': type.index});
  ...
  Future<Result<Map<String, dynamic>>> doRequest(
    ...
    try {
      ...
      final convertedParams = param.map((k, v) => MapEntry(k.toString(), v.toString()));
